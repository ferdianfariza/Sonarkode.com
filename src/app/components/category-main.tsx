...
export default function CategoryMain() {
  ...
  useEffect(() => {
    ...
    const result = posts.map((post, index) => {
      const preferredLayout = Math.floor(index / 2) % 2 === 0 ? "image" : "text";
      const layout: "image" | "text" = preferredLayout === "image" && post.image ? "image" : "text";
      return { ...post, layout };
    });
  ...
  return (
   <>
   <h1 className="text-3xl font-bold text-center w-full mb-8">{structuredPosts[0]?.category}</h1>
   <div className="container mx-auto gap-8 lg:gap-12 max-w-[1165px] grid grid-cols-1 sm:grid-cols-2 place-items-center">
      {structuredPosts.map((post) => (
         <div key={post._id} className="w-full flex flex-col justify-between h-auto md:h-[580px] border-b border-ui">
          <div className="flex flex-col space-y-4">
            ...
            <p className="font-medium uppercase tracking-widest text-zinc-600 dark:text-zinc-400 text-[12px]">
              <span className=" font-semibold">{post.category}</span> - {post.readtime}
            </p>
            <Link
              href={post.url}
              className={`font-semibold dark:text-gray-100 tracking-tight hover:underline ${
                post.layout === "text" ? "line-clamp-5 text-2xl md:text-3xl leading-10" : "text-xl  line-clamp-3 leading-8"
              }`}
            >
              {post.title}
            </Link>
            <p className={`text-[15px] text-gray-600 dark:text-gray-400 leading-5  ${
